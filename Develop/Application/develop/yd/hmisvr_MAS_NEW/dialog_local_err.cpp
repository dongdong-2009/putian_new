...
void dialog_local_err::init_page_val()
{
    ...
    */
    ...
    if(local_errsum > 0)
    {
        ...
        errtype_num_1 = local_array_errfile[local_errsum - lbl_num].err_protcl_type;
        errtype_num_2 = local_array_errfile[local_errsum - lbl_num_2].err_protcl_type;
        errtype_num_3 = local_array_errfile[local_errsum - lbl_num_3].err_protcl_type;

        //&local_tempsub2_buf[0] 存了第一条故障记录的第一个故障
        memset(local_tempsub2_buf,0,sizeof(local_tempsub2_buf));
        local_tempadd_sub2 = 0;
        Err_local_analysis(err_code,errtype_num_1);
        ...
        memset(local_tempsub2_buf,0,sizeof(local_tempsub2_buf));
        local_tempadd_sub2 = 0;
        Err_local_analysis(err_code_2,errtype_num_2);
        ...
        memset(local_tempsub2_buf,0,sizeof(local_tempsub2_buf));
        local_tempadd_sub2 = 0;
        Err_local_analysis(err_code_3,errtype_num_3);
...
int Err_localfile_analysis(const char *file_name, struct _partbuf_0x97  *partbuf_0x97_OutPt)
{
    ...
    while (!feof(fp_file))
    {
        ...
        for(startpos=0; startpos<readLen; startpos++)
        {
            ...
            if(linedatabuf[startpos]==0x0d||linedatabuf[startpos]==0x0a)    // \r and \n
            {
                //break;
                continue;
            }
            if(linedatabuf[startpos]==0x7c)                                 //"|"
            {
                ...
                if(Sectionindex==0)                                         // err type code
                {
                    tempeventtype=atoi((char *)SectionBuf);
                    if(tempeventtype== 1)                                   // 1 -> 0x11, 2 -> 0x31
                    {
...
//local_tempsub2_buf: 用来存储一条记录中可能出现的多条故障
...
int   Err_local_analysis(int error_state_flag,int err_type_index)
{
    if(language_num == 1)
    {
        ...
        if( (error_state_flag & (1<<0)) )     //
        {
            if(err_type_index == 1)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"读卡器通信故障",sizeof("读卡器通信故障"));
            }
            else if(err_type_index == 2)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"绝缘电阻值小于阀值",sizeof("绝缘电阻值小于阀值"));
            }
            else
            {
            }
        ...
        if( (error_state_flag & (1<<1)) )
        {
            if(err_type_index == 1)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"读卡器与主控板通信超时",sizeof("读卡器与主控板通信超时"));
            }
            else if(err_type_index == 2)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"绝缘模块故障",sizeof("绝缘模块故障"));
            }
            else
            {
            }
        ...
        if( (error_state_flag & (1<<2)) )
        {
            if(err_type_index == 1)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"显示屏硬件损坏",sizeof("显示屏硬件损坏"));
            }
            else if(err_type_index == 2)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"进风口温度过高",sizeof("进风口温度过高"));
            }
            else
            {
            }
        ...
        if( (error_state_flag & (1<<3)) )
        {
            if(err_type_index == 1)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"显示屏与主控板通信超时",sizeof("显示屏与主控板通信超时"));
            }
            else if(err_type_index == 2)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"输 入 过 压",sizeof("输 入 过 压"));
            }
            else
            {
            }
        ...
        if( (error_state_flag & (1<<4)))
        {
            if(err_type_index == 1)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"交流接触器状态监测故障",sizeof("交流接触器状态监测故障"));
            }
            else if(err_type_index == 2)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"输 入 欠 压",sizeof("输 入 欠 压"));
            }
            else
            {
            }
        ...
        if((error_state_flag & (1<<5)))
        {
            if(err_type_index == 1)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"交流接触器供电故障",sizeof("交流接触器供电故障"));
            }
            else if(err_type_index == 2)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"输 入 缺 相",sizeof("输 入 缺 相"));
            }
            else
            {
            }
        ...
        if((error_state_flag & (1<<6)))
        {
            if(err_type_index == 1)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"低压电源异常",sizeof("低压电源异常"));
            }
            else if(err_type_index == 2)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"输 入 过 流",sizeof("输 入 过 流"));
            }
            else
            {
            }
        ...
        if( (error_state_flag & (1<<7)) )
        {
            if(err_type_index == 1)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"主控板硬件故障",sizeof("主控板硬件故障"));
            }
            else if(err_type_index == 2)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"谐波超限（预留）",sizeof("谐波超限（预留）"));
            }
            else
            {
            }
        ...
        if( (error_state_flag & (1<<8)) )
        {
            if(err_type_index == 1)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"主板软件故障",sizeof("主板软件故障"));
            }
            else if(err_type_index == 2)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"输 出 过 压",sizeof("输 出 过 压"));
            }
            else
            {
            }
        ...
        if( (error_state_flag & (1<<9)) )
        {
            if(err_type_index == 1)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"控制板与模块通讯不稳定",sizeof("控制板与模块通讯不稳定"));
            }
            else if(err_type_index == 2)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"输 出 欠 压",sizeof("输 出 欠 压"));
            }
            else
            {
            }
        ...
        if( (error_state_flag & (1<<10)) )
        {
            if(err_type_index == 1)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"模块通讯接收端故障",sizeof("模块通讯接收端故障"));
            }
            else if(err_type_index == 2)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"输 出 过 流",sizeof("输 出 过 流"));
            }
            else
            {
            }
        ...
        if( (error_state_flag & (1<<11)) )
        {
            if(err_type_index == 1)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"数据传输丢包-1级",sizeof("数据传输丢包-1级"));
            }
            else if(err_type_index == 2)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"输 出 短 路",sizeof("输 出 短 路"));
            }
            else
            {
            }
        ...
        if( (error_state_flag & (1<<12)) )
        {
            ...
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"充电模块无输出电压、电流",sizeof("充电模块无输出电压、电流"));
            }
            else
            {
            }
        ...
        if( (error_state_flag & (1<<16)) )
        {
            if(err_type_index == 1)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"键盘硬件损坏",sizeof("键盘硬件损坏"));
            }
            else if(err_type_index == 2)
            {
        ...
        if( (error_state_flag & (1<<17)) )
        {
            if(err_type_index == 1)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"键盘与主控板通信故障",sizeof("键盘与主控板通信故障"));
            }
            else if(err_type_index == 2)
            {
            }
        ...
        if( (error_state_flag & (1<<19)) )
        {
            if(err_type_index == 1)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"电表硬件故障",sizeof("电表硬件故障"));
            }
            else if(err_type_index == 2)
            {
        ...
        if( (error_state_flag & (1<<20)) )
        {
            if(err_type_index == 1)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"电表与主控板通信故障",sizeof("电表与主控板通信故障"));
            }
            else if(err_type_index == 2)
            {
            }
        ...
        if( (error_state_flag & (1<<22)) )
        {
            if(err_type_index == 1)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"数据传输丢包-2级",sizeof("数据传输丢包-2级"));
            }
            else if(err_type_index == 2)
            {
        ...
        if( (error_state_flag & (1<<24)) )
        {
            ...
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"风 扇 故 障",sizeof("风 扇 故 障"));
            }
            else
            {
            }
        ...
        if( (error_state_flag & (1<<25)) )
        {
            if(err_type_index == 1)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"读卡器与主控板通信超时故障",sizeof("读卡器与主控板通信超时故障"));
            }
            else if(err_type_index == 2)
            {
            }
        ...
        if( (error_state_flag & (1<<26)) )
        {
            if(err_type_index == 1)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"打印机硬件损坏",sizeof("打印机硬件损坏"));
            }
            else if(err_type_index == 2)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"存储器读取数据失败",sizeof("存储器读取数据失败"));
            }
            else
            {
            }
        ...
        if( (error_state_flag & (1<<27)) )
        {
            if(err_type_index == 1)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"打印机与主控板通信故障",sizeof("打印机与主控板通信故障"));
            }
            else if(err_type_index == 2)
            {
        ...
        if( (error_state_flag & (1<<28)) )
        {
            if(err_type_index == 1)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"打印机缺纸状态（新增）",sizeof("打印机缺纸状态（新增）"));
            }
            else if(err_type_index == 2)
            {
            }
        ...
        if( (error_state_flag & (1<<29)) )
        {
            if(err_type_index == 1)
            {
                memcpy(&local_tempsub2_buf[local_tempadd_sub2++],"数据传输丢包-3级",sizeof("数据传输丢包-3级"));
            }
            else if(err_type_index == 2)
            {
            }
